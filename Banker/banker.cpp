...
//run through all unterminated resources to check whether this request is safe
bool checkSafe(int self, int resource, int request){
    bool safe = false;
    int available = resources[resource]-request;
    FOR(i, 0, taskNum){
        int need;
        
        if(i==self) need = claims[i][resource]-request;
        else need = claims[i][resource];
        
        if(status[i]!=1 && need<=available) safe = true;
    }
    return safe;
}
    ...
    while(true){
        ...
        while(blockingNum--){
            ...
            if(status[i]!=1){
                ...
                bool safe = checkSafe(taskIndex,resourceIndex,requestQuantity);
                if(safe){
        ...
        for(int i=0;i<taskNum;i++){
            ...
            if(instruction!="void"){
                ...
                if(order=="initiate"){
                    ...
                    //error check is required for Banker
                    if(requestQuantity>resources[resourceIndex]){
                        printf("The initial claim exceeds system resources. Aborting task %d.",taskId);
                        status[taskIndex]=1;
                    }
                    ...
                    //the amount of free resource is able to terminate a task then grant
                    bool safe = checkSafe(taskIndex,resourceIndex,requestQuantity);
                    if(safe){
                        //check whether go beyond max claim
                        int newClaim = claims[taskIndex][resourceIndex]-requestQuantity;
                        if(newClaim<0){
                            //Abort task and release resources
                            printf("Task %d's request exceed its max claim, Aborting\n",taskId);
                            status[taskIndex]=1;
                            FOR(j,0,typeNum){
                                //release resources
                            ...
                            resources[resourceIndex]-=requestQuantity;
                            holding[taskIndex][resourceIndex]+=requestQuantity;
                            //claims represent the additional needs
                            claims[taskIndex][resourceIndex]-=requestQuantity;
                            vecOfInterators[taskIndex]++;
                            printf("Grants request of %d resource type %d to task %d\n",requestQuantity,resourceType,taskId);
                        }
                        ...
                        //unsafe, do not grant request and block
                        status[taskIndex]=3;
                        waiting[taskIndex]++;
